...
{
	name = "[PropTricks] Chat Processor",
	author = "Haze",
...
public void OnPluginStart()
{
	...
	LoadTranslations("proptricks-chat.phrases");
	...
	gCV_RankingsIntegration = new Convar("proptricks_chat_rankings", "1", "Integrate with rankings?\n0 - Disabled\n1 - Enabled", 0, true, 0.0, true, 1.0);
	gCV_CustomChat = new Convar("proptricks_chat_customchat", "1", "Allow custom chat names or message colors?\n0 - Disabled\n1 - Enabled (requires chat flag/'proptricks_chat' override)\n2 - Allow use by everyone", 0, true, 0.0, true, 2.0);
	gCV_Colon = new Convar("proptricks_chat_colon", ":", "String to use as the colon when messaging.");
	...
	gH_ChatCookie = RegClientCookie("proptricks_chat_selection", "Chat settings", CookieAccess_Protected);
...
public void OnMapStart()
{
	if(!LoadChatConfig())
	{
		SetFailState("Could not load the chat configuration file. Make sure it exists (addons/sourcemod/configs/proptricks-chat.cfg) and follows the proper syntax!");
	}

	if(!LoadChatSettings())
	{
		SetFailState("Could not load the chat settings file. Make sure it exists (addons/sourcemod/configs/proptricks-chatsettings.cfg) and follows the proper syntax!");
	}
}
...
bool LoadChatConfig()
{
	...
	BuildPath(Path_SM, sPath, PLATFORM_MAX_PATH, "configs/proptricks-chat.cfg");

	KeyValues kv = new KeyValues("proptricks-chat");
...
{
	char sPath[PLATFORM_MAX_PATH];
	BuildPath(Path_SM, sPath, PLATFORM_MAX_PATH, "configs/proptricks-chatsettings.cfg");
	...
	if(!kv.JumpToKey("CS:S") || !kv.GotoFirstSubKey(false))
	{
		SetFailState("Invalid \"configs/proptricks-chatsettings.cfg\" file, or the game section is missing");
	}
...
public Action Command_CCHelp(int client, int args)
{
	...
	PropTricks_PrintToChat(client, "%T", "CheckConsole", client);
...
public Action Command_CCName(int client, int args)
{
	...
	if(!HasCustomChat(client))
	{
		PropTricks_PrintToChat(client, "%T", "NoCommandAccess", client);
	...
	{
		PropTricks_PrintToChat(client, "%T", "ArgumentsMissing", client, "sm_ccname <text>");
		PropTricks_PrintToChat(client, "%T", "ChatCurrent", client, gS_CustomName[client]);
	...
	{
		PropTricks_PrintToChat(client, "%T", "NameOff", client, sArgs);
	...
	PropTricks_PrintToChat(client, "%T", "ChatUpdated", client);
...
public Action Command_CCMessage(int client, int args)
{
	...
	if(args == 0 || strlen(sArgs) == 0)
	{
		PropTricks_PrintToChat(client, "%T", "ArgumentsMissing", client, "sm_ccmsg <text>");
		PropTricks_PrintToChat(client, "%T", "ChatCurrent", client, gS_CustomMessage[client]);
	...
	{
		PropTricks_PrintToChat(client, "%T", "MessageOff", client, sArgs);
...
public int MenuHandler_ChatRanks(Menu menu, MenuAction action, int param1, int param2)
{
	if(action == MenuAction_Select)
	{
		...
		PropTricks_PrintToChat(param1, "%T", "ChatUpdated", param1);
...
{
	return (gCV_CustomChat.IntValue > 0 && (CheckCommandAccess(client, "proptricks_chat", ADMFLAG_CHAT) || gCV_CustomChat.IntValue == 2));
}
...
bool HasRankAccess(int client, int rank)
{
	...
	float fRank = (cache.iRangeType != Rank_Points)? float(PropTricks_GetRank(client)):PropTricks_GetPoints(client);
	...
	{
		int iRanked = PropTricks_GetRankedPlayers();
...
public Action Command_CCList(int client, int args)
{
	...
	for(int i = 1; i <= MaxClients; i++)
	{
		...
		if(gCV_CustomChat.IntValue > 0 && (CheckCommandAccess(i, "proptricks_chat", ADMFLAG_CHAT) || gCV_CustomChat.IntValue == 2))
		{
...
void FormatChat(int client, char[] buffer, int size)
{
	...
	if(gB_Rankings)
	{
		int iRank = PropTricks_GetRank(client);
		char sRank[16];
...
void SQL_DBConnect()
{
	...
	{
		FormatEx(sQuery, 512,
			"CREATE TABLE IF NOT EXISTS `chat` (`auth` INT NOT NULL, `name` INT NOT NULL DEFAULT 0, `ccname` VARCHAR(128) COLLATE 'utf8mb4_unicode_ci', `message` INT NOT NULL DEFAULT 0, `ccmessage` VARCHAR(16) COLLATE 'utf8mb4_unicode_ci', PRIMARY KEY (`auth`), CONSTRAINT `ch_auth` FOREIGN KEY (`auth`) REFERENCES `users` (`auth`) ON UPDATE CASCADE ON DELETE CASCADE) ENGINE=INNODB;");
	}

	else
	{
		FormatEx(sQuery, 512,
			"CREATE TABLE IF NOT EXISTS `chat` (`auth` INT NOT NULL, `name` INT NOT NULL DEFAULT 0, `ccname` VARCHAR(128), `message` INT NOT NULL DEFAULT 0, `ccmessage` VARCHAR(16), PRIMARY KEY (`auth`), CONSTRAINT `ch_auth` FOREIGN KEY (`auth`) REFERENCES `users` (`auth`) ON UPDATE CASCADE ON DELETE CASCADE);");
	}
...
void SaveToDatabase(int client)
{
	...
	char sQuery[512];
	FormatEx(sQuery, 512,
		"REPLACE INTO chat (auth, name, ccname, message, ccmessage) VALUES (%d, %d, '%s', %d, '%s');",
		iSteamID, gB_NameEnabled[client], sEscapedName, gB_MessageEnabled[client], sEscapedMessage);
...
void LoadFromDatabase(int client)
{
	...
	char sQuery[256];
	FormatEx(sQuery, 256, "SELECT name, ccname, message, ccmessage FROM chat WHERE auth = %d;", iSteamID);
